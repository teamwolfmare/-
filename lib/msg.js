...
const sms = (princessOlya, m) => {
    if (m.key) {
        m.id = m.key.id
        m.chat = m.key.remoteJid
        m.fromMe = m.key.fromMe
        m.isGroup = m.chat.endsWith('@g.us')
        m.sender = m.fromMe ? princessOlya.user.id.split(':')[0]+'@s.whatsapp.net' : m.isGroup ? m.key.participant : m.key.remoteJid
    }
    if (m.message) {
        ...
        if (m.msg) {
            ...
            if (m.quoted) {
                ...
                m.quoted.fromMe = m.quoted.sender.split('@')[0].includes(princessOlya.user.id.split(':')[0])
                ...
                m.quoted.delete = () => princessOlya.sendMessage(m.chat, { delete: m.quoted.fakeObj.key })
                m.quoted.react = (emoji) => princessOlya.sendMessage(m.chat, { react: { text: emoji, key: m.quoted.fakeObj.key } })
            }
        }
        m.download = (filename) => downloadMediaMessage(m, filename)
    }
    
    m.reply = (teks, id = m.chat, option = { mentions: [m.sender] }) => princessOlya.sendMessage(id, { text: teks, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
    m.react = (emoji) => princessOlya.sendMessage(m.chat, { react: { text: emoji, key: m.key } })
